TOR SUPPORT IN WMC CORE
=======================

It is possible to run Wmc Core as a Tor hidden service, and connect to such services.
...
1. Run Wmc Core behind a Tor proxy
----------------------------------

The first step is running Wmc Core behind a Tor proxy. This will already make all
...
2. Run a Wmc Core hidden server
-------------------------------
...
config file):
	...
	HiddenServicePort 13052 127.0.0.1:13052
	HiddenServicePort 13054 127.0.0.1:13054

The directory can be different of course, but (both) port numbers should be equal to
your dashd's P2P listen port (13052 by default).

	-externalip=X   You can tell Wmc Core about its publicly reachable address using
...
and open port 13052 on your firewall (or use -upnp).
...
3. List of known Wmc Core Tor relays
------------------------------------
...
Wmc Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Wmc Core automatically creates a hidden service to listen on, without
manual configuration. This will positively affect the number of available
.onion nodes.

This new feature is enabled by default if Wmc Core is listening, and
