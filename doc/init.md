...
see `contrib/debian/examples/wmc.conf`.
...
Configuration file:  `/etc/dashcore/wmc.conf`  
...
dashcore user and group.  Access to wmc-cli and other dashd rpc clients
can then be controlled by group membership.

3b) Mac OS X

Binary:              `/usr/local/bin/dashd`  
Configuration file:  `~/Library/Application Support/DashCore/wmc.conf`  
...
Copy org.wmc.dashd.plist into ~/Library/LaunchAgents. Load the launch agent by
running `launchctl load ~/Library/LaunchAgents/org.wmc.dashd.plist`.
...
You will need to modify org.wmc.dashd.plist if you intend to use it as a
