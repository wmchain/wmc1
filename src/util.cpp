...
const char * const BITCOIN_CONF_FILENAME = "wmc.conf";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "wmc" is a composite category enabling all Wmc-related debug output
            if(ptrCategory->count(string("wmc"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty wmc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override wmc.conf
