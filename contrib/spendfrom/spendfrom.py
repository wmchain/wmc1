...
#
# Assumes it will talk to a dashd or Wmc-Qt running
# on localhost.
#
# Depends on jsonrpc
#
...
def determine_db_dir():
    """Return the default location of the Wmc Core data directory"""
...
def read_bitcoin_config(dbdir):
    """Read the wmc.conf file from dbdir, returns dictionary of settings"""
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "wmc.conf"))))
    return dict(config_parser.items("all"))

def connect_JSON(config):
    """Connect to a Wmc Core JSON-RPC server"""
    testnet = config.get('testnet', '0')
    testnet = (int(testnet) > 0)  # 0/1 in config file, convert to True/False
    if not 'rpcport' in config:
        config['rpcport'] = 13053 if testnet else 13051
...
def list_available(dashd):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-wmc-address
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of wmc.conf file with RPC username/password (default: %default)")
